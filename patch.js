...
// Optimize dissolveHero
const dissolveHeroOld = `  const dissolveHero = ()=>{
...
const dissolveHeroNew = `  let dissolveRaf = 0;
...
code = code.replace(dissolveHeroOld, dissolveHeroNew);

// Add will-change to css for nova-pixie-chat and mascot and engineering-editor
code = code.replace(`.nova-pixie-mascot{
  width:64px;
  height:64px;`, `.nova-pixie-mascot{
  will-change: transform, box-shadow;
  width:64px;
  height:64px;`);

code = code.replace(`.nova-pixie-chat{
  position:fixed;
  bottom:96px;`, `.nova-pixie-chat{
  will-change: transform, opacity;
  position:fixed;
  bottom:96px;`);

// Optimize mousemove raf
...
code = code.replace(mousemoveOld, mousemoveNew);
